...
#include "envelope.h"       // env_max::InputTriangle, compute_lower_envelope_diagram, register_envelope_diagram_edges_2d
#include "dedekind_poset.h" // dm_completion::DedekindPoset
...
// Last known enabled states (optional; helps after rebuild)
    ...
    std::vector<std::array<int,3>>& F_out)
{
  ...
  // match compare_nodes scaling conventions
  float S = 1.0f;
...
// Collect lifted triangles (in envelope coords) from P1 nodes (same lifting as compare/dedekind envelope UIs)
                                 ...
                                 float zSpacing)
{
  ...
  // Determine old/new per your desired rule:
  // old node <=> |min(F)| == 1
  ...
  // grid nodes (returns grid positions)
                            ...
                            const dm_completion::DedekindPoset& C)
{
  ...
  const float cell_scale = 0.08f;   // same as poset_vis meshScale
  const float y_plane    = 0.0f;
  const float radius     = 0.0015f;
  const glm::vec3 diagColor = glm::vec3(0.f, 0.f, 0.f);
  ...
  for (int cutId = 0; cutId < (int)C.cuts.size(); ++cutId) {
    ...
    if ((int)minF.size() == 1) {
      // old: show triangulation of the single minF node
