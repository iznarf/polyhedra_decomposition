...
static void build_completion_vis() {
    ...
    std::vector<glm::vec3> pos = pst_vis::register_poset_as_grid_colored(node_names,M,g_completion.levels,center,g_compXSpacing,g_compZSpacing,colors,0.01f);

    // edges in black 
    pst_vis::register_cover_up_as_edges(edge_names, pos, g_completion.cover_up, glm::vec3(0.f, 0.f, 0.f));

    // toggles
    if (polyscope::hasPointCloud(node_names))   polyscope::getPointCloud(node_names)->setEnabled(g_showCompletionNodes);
    if (polyscope::hasCurveNetwork(edge_names)) polyscope::getCurveNetwork(edge_names)->setEnabled(g_showCompletionEdges);
  }
