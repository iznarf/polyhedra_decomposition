...
namespace df {
    ...
    static std::unordered_set<df::vertex_id> collect_ids(const df::Tri2& tri) {
    ...
    std::vector<df::vertex_id> find_missing_vertices(const df::Tri2& current, const df::Tri2& target) {
        ...
        for (auto v = target.finite_vertices_begin(); v != target.finite_vertices_end(); ++v) {
    ...
    void apply_vertex_insertion(df::vertex_id id, df::InputData& D) {
        ...
        if (lt == df::Tri2::FACE) {
            ...
            df::Tri2::Vertex_handle vh = T.insert(p, fh);
